...
import {
  ...
  type ExportSheetLayoutOptions,
  type GeneratedArtifacts,
  type GeneratedSvgPage,
  type SvgPerforationOptions
} from './export-contracts';
import { applySvgPerforation } from './svg-perforation';
...
export function filterTemplateLayers(
  ...
  // Empty layer selection means "keep geometry as-is".
  if (!layers.length) {
    return geometry;
  }
...
// -----------------------------
// Split boundary closure helpers
// -----------------------------
...
// -----------------------------
// Packing primitives and metrics
// -----------------------------
...
function mergeTinyPackableComponents(
  ...
  // Merge smallest fragments first to keep deterministic merge behavior.
...
function choosePackingPlacement(
  ...
  // Best-fit style placement over current free rectangles.
...
function applyPackingPlacement(
  ...
  // Split intersecting free rectangles around the used rectangle, then normalize.
...
// -----------------------------
// SVG page rendering + assembly guide
// -----------------------------
...
// -----------------------------
// Orientation search before splitting
// -----------------------------
...
function chooseBestOrientation(
  ...
  // Brute-force sweep keeps this deterministic and easy to validate.
...
// -----------------------------
// Layout strategies:
// - legacy deterministic grid split
// - split-pack optimized flow
// -----------------------------
function buildSvgPagesForLayoutLegacy(
  ...
  // Build strict row/column grid pages first; packing optimization is optional later.
